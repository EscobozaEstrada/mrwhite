...
def health_chat():
    ...
    Expected JSON:
    {
        "message": "string",
        "pet_id": int (optional),
        "include_records": boolean (optional, default true),
        "include_insights": boolean (optional, default true)
    }
    """
    try:
        user_id = get_jwt_identity()
        ...
        if include_records:
            ...
            if records:
                ...
                for record in records[:10]:  # Limit context size
                    context_parts.append(
                        f"- {record.record_date}: {record.record_type.value.title()} - {record.title}"
                    )
        ...
        if include_insights:
            ...
            if insights:
                context_parts.append("\nHealth Insights:")
                for insight in insights[:5]:  # Limit context size
        ...
        prompt = f"""
        You are a knowledgeable pet health assistant. Answer the user's question based on their pet's health history and provide helpful advice.
        ...
        Please provide a helpful, accurate response based on the available health data. If you need more specific information, ask clarifying questions.
        """
        
        # TODO: Replace with actual AI chat service
        # For now, return a placeholder response
        response_message = f"I understand you're asking about: '{message}'. Based on your pet's health records, I can provide personalized advice. This feature will be fully implemented with AI integration."
        
        return jsonify({
            'response': response_message,
            'context_used': len(context_parts) > 0,
            'record_count': len(records) if include_records and 'records' in locals() else 0,
            'insight_count': len(insights) if include_insights and 'insights' in locals() else 0
        }), 200
